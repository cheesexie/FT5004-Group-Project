...
contract Dice {
    ...
    struct dice {
        ...
        address owner;
        address prevOwner;
        uint256 luckyTimes; //new
    }
    
    event rolling (uint256 diceId);
    event rolled (uint256 diceId, uint8 newNumber);
    event luckyTimesEvent(uint256 diceId, uint256 luckyTimes);  //new
    ...
    function stopRoll(uint256 diceId) public ownerOnly(diceId) validDiceId(diceId) {
            ...
            emit luckyTimesEvent(diceId, dices[diceId].luckyTimes); //new
        }
    ...
    //new destroyDice function
    function destroyDice(uint256 diceId) public ownerOnly(diceId) validDiceId(diceId) {
        uint256 diceValue = dices[diceId].creationValue;
        address payable diceOwner = address(uint160(dices[diceId].owner));
        delete dices[diceId];
        diceOwner.transfer(diceValue);
    }
    function getDiceOwner(uint256 diceId) public view validDiceId(diceId) returns (address) {
        return dices[diceId].owner;
    }
    function getDiceCreationValue(uint256 diceId) public view validDiceId(diceId) returns (uint256) {
        return dices[diceId].creationValue;
    }
    function updateDiceCreationValue(uint256 diceId, uint256 newValue) public validDiceId(diceId) {
        dices[diceId].creationValue = newValue;
    }
}
